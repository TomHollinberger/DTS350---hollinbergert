...
a <- gapminder
#Find Kuwait before deleting it's row.
(filter(a, country == "Kuwait"))
(filter(gapminder, country != "Kuwait"))


dfa <- filter(gapminder, country != "Kuwait")
dfa <- mutate(dfa, pop = pop/100000)  #mutate overwrites the variabel if you use the same variable name on both sides of the equal sign.
str(dfa)
#tibble [1,692 x 6] (S3: tbl_df/tbl/data.frame)
#$ country  : Factor w/ 142 levels "Afghanistan"   
#Interesting, the number of rows is lower 1704 to 1692 bc of Kuwait removed
# But the number of levels is the same at 142.
...
#first, need to create wtd average for each continent_colors


graphAdata <- group_by(dfa, year, continent)
graph2data <- (summarize(graphAdata, wtgdp = weighted.mean(gdpPercap, pop, na.rm = TRUE), contsum = sum(pop, na.rm = TRUE)))
graphAdata <- group_by(dfa, year, continent)
graph2data <- (summarize(graphAdata, wtgdp = weighted.mean(gdpPercap, pop, na.rm = TRUE), contsum = sum(pop, na.rm = TRUE)))
graph2data
...
ggplot() +
  ...
  geom_point(data = graph2data, mapping = aes(x = year, y = wtgdp, size = contsum),color = "black") +  #group=country is like series=country in Excel
